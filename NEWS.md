...
* `trace_biosynthesis()` and `trace_biosynthesis_virtual()` now add a logical `target` vertex attribute indicating which nodes are target glycans; `autoplot()` uses it to highlight targets by default in multi-target networks, with explicit control through `highlight_target`. (#37, #38)
