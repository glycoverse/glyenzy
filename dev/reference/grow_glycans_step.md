...
`grow_glycans_step()` performs one round of enzyme action, while
`grow_glycans()` performs multiple rounds. The only difference between
`grow_glycans_step()` and `grow_glycans(n_steps = 1)` is that the latter
returns the original input glycans as well. For both, a vector of unique
glycan structures is returned.

The number of glycans generated by `grow_glycans()` is typically
