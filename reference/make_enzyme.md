...
(GHs), and sulfotransferases (STs) with this function. Special enzymes
like starter GTs for glycosylation initiation are reserved for
predefined enzymes.
...
- rules:
  ...
  - `requires`: An optional list of requirement entries, each with
    `motif` and `alignment`. Defaults to an empty list.

- type:

  The type of the enzyme: `"GT"`, `"GH"`, or `"ST"`.
...
For sulfotransferases, the product must have the same residue and
linkage topology as the acceptor and add exactly one sulfate
substituent.

### requires

`requires` provides optional context motifs. Each entry contains `motif`
and `alignment`; requirements are OR-valued, so at least one must match.
An empty list means no additional context is required. Requirement
motifs use sulfate-subset semantics, so an unsulfated requirement also
matches the same topology with additional sulfates. Acceptor and reject
motifs remain exact with respect to substituents during enzyme
application.
