...
#'
#' You can create regular glycosyltransferases (GTs), glycoside hydrolases
#' (GHs), and sulfotransferases (STs) with this function.
...
#'
#' For sulfotransferases, the product must have the same residue and linkage
#' topology as the acceptor and add exactly one sulfate substituent.
#'
#' ## requires
#'
#' `requires` provides optional context motifs. Each entry contains `motif` and
#' `alignment`; requirements are OR-valued, so at least one must match. An empty
#' list means no additional context is required. Requirement motifs use
#' sulfate-subset semantics, so an unsulfated requirement also matches the same
#' topology with additional sulfates. Acceptor and reject motifs remain exact
#' with respect to substituents during enzyme application.
#'
...
#'   - `requires`: An optional list of requirement entries, each with `motif`
#'     and `alignment`. Defaults to an empty list.
...
.create_rule <- function(rule_data) {
  ...
  checkmate::assert_list(requires_data)
  requires <- purrr::map(requires_data, function(requirement) {
    checkmate::assert_list(requirement, names = "named")
    if (
      length(requirement) != 2L ||
        !setequal(names(requirement), c("motif", "alignment"))
    ) {
      cli::cli_abort(
        "Each {.field requires} entry must contain only {.field motif} and {.field alignment}."
      )
    }
    motif <- .process_glycan_arg(requirement$motif)
