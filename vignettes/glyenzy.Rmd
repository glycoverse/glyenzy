...
Think of glycan biosynthesis as nature's most sophisticated assembly line 🏭.
...
Enter `glyenzy` 🧬 – your computational toolkit for exploring this fascinating world.
...
**Quick heads-up:** `glyenzy` stands on the shoulders of giants – specifically `glyrepr`, `glyparse`, and `glymotif`.
You don't need to master these packages to get started, but they're worth exploring for advanced glycan wizardry ✨.
Also, this vignette assumes you're comfortable with IUPAC-condensed notation.
New to it? No worries – check out [this friendly tutorial](https://glycoverse.github.io/glyrepr/articles/iupac.html) first!
...
Time to meet your new best friend: `find_enzyme()` 👋
...
Let's give ST3GAL1 a chance to work its magic:

```{r}
apply_enzyme(glycan, "ST3GAL1")
```

Fascinating! 🎯 The function returns a `glyrepr::glycan_structure()` vector,
but the real story is what happened: a shiny new sialic acid got attached to the β1-3 Gal,
while the β1-4 Gal was completely ignored.
Why? Because ST3GAL1 is incredibly picky – it only recognizes "Gal(β1-3)GalNAc(α1-" as its substrate.
...
You've already met `find_enzyme()` – perfect for getting the full cast of characters.
But sometimes you want more targeted intel:

- `have_enzyme()` answers "Was enzyme X involved?" with a simple yes/no
- `count_enzyme()` tells you exactly how many times an enzyme got busy

This intel is gold for multiomics analysis – imagine linking glycan structures
directly to enzyme expression levels! 📊

Ready for the grand finale? Meet `trace_biosynthesis()` –
...
What you get back is a directed `igraph` object – think of it as a molecular family tree! 🌳
...
**Note:** The plotting could use some TLC – we're working on prettier visualizations! 🎨

## Crystal Ball Mode: Predicting Glycan Futures 🔮

Now for the really fun part – playing molecular fortune teller!
...
Boom! 💥 We just witnessed the birth of the famous Core 1 O-GalNAc glycan!
...
This is biochemical branching in action! 🌿
...
or `grow_glycans_step()` if you prefer to watch the drama unfold step by step:
...
- **ST3GAL3**: Decorates with α2-3 sialic acid (the fancy finishing touch)
...
the results might not be accurate. We're basically specialists, not generalists! 🎯
...
Think of it as getting a list of suspects rather than the definitive perpetrator! 🕵️

### Concrete Residues Only

glyenzy is picky about precision – it only works with **concrete** residues
like "Glc" and "GalNAc", not **generic** ones like "Hex" or "HexNAc".
If your data uses generic terms, you'll need to be more specific! 🎯
