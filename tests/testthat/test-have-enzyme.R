...
test_that("have_enzyme works for `glyrepr_structure` and `glyenzy_enzyme`", {
  ...
  expect_false(have_enzyme(glycan, enzyme))
})
...
test_that("have_enzyme works vectorizedly", {
  ...
  expect_equal(have_enzyme(glycans, "ST3GAL3"), c(FALSE, FALSE))
})
...
test_that("have_enzyme works for POGLUT2", {
  expect_true(have_enzyme("Xyl(a1-3)Glc(b1-", "POGLUT2"))
...
test_that("have_enzyme path method treats N-glycan precursor GTs specially", {
  expect_true(suppressMessages(
    have_enzyme(glyrepr::n_glycan_core(), "ALG10", method = "path")
  ))
  expect_false(suppressMessages(
    have_enzyme(glyrepr::o_glycan_core_1(), "ALG10", method = "path")
  ))
})

test_that("have_enzyme path method supports custom enzyme objects", {
  enz <- make_enzyme(
    name = "TEST_ST3GAL",
    type = "GT",
    species = "human",
  ...
  expect_true(suppressMessages(
    have_enzyme("Neu5Ac(a2-3)Gal(b1-3)GalNAc(a1-", enz, method = "path")
  ))
})
